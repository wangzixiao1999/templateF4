...
int32_t yt8512c_get_link_state(yt8512c_object_t *pobj)
{
    ...
    /* Read Basic Status Register (Register 1) to check Link Status */
    if (pobj->io.readreg(pobj->devaddr, YT8512C_BSR, &bsrval) < 0)
    {
        return YT8512C_STATUS_READ_ERROR;
    }

    /* Check if Link is Up (Bit 2 of BSR) */
    if ((bsrval & YT8512C_BSR_LINK_STATUS) == 0)
    {
    ...
    uint32_t speed = readval & YT8512C_PHYSCSR_SPEED_MASK;
    uint32_t duplex = readval & YT8512C_PHYSCSR_DUPLEX;

    if (speed == YT8512C_PHYSCSR_SPEED_100M)
    {
        if (duplex)
    ...
    else /* 10M (or others, treating as 10M default if not 100M) */
    {
        if (duplex)
